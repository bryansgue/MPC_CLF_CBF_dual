...
def main(vel_pub, vel_msg, odom_sim_pub_2, odom_sim_msg_2):
    ...
    # Read Real data
    x[:, 0] = get_odometry_simple_quat_2()
    ...
    for k in range(0, t.shape[0]-N_prediction):
        ...
        # SET OBSTACLE
        matrice_1 = get_odometry_simple_quat_1()
        
        nx_obs = 1* matrice_1[0]
        ny_obs = 1* matrice_1[1]
        ...
        publish_matrix(simX[0:3, 0:N_prediction], '/Prediction_2')
        ...
        if opcion == "Real":
            x[:, k+1] = get_odometry_simple_quat_2()
        elif opcion == "Sim":
            x[:, k+1] = f_d(x[:, k], u_control[:, k], t_s, f)
            pub_odometry_sim_quat(x[:, k+1], odom_sim_pub_2, odom_sim_msg_2)
        else:
...
if __name__ == '__main__':
    try:
        ...
        # SUCRIBER
        velocity_subscriber = rospy.Subscriber("/M2/dji_sdk/odometry", Odometry, odometry_call_back_2)
        Matrice100_1 = rospy.Subscriber("/dji_sdk/odometry", Odometry, odometry_call_back_1)

        
        # PUBLISHER
        velocity_message = TwistStamped()
        velocity_publisher = rospy.Publisher("/M2/m100/velocityControl", TwistStamped, queue_size=10)

        odometry_sim_msg_2 = Odometry()
        odom_sim_pub_2 = rospy.Publisher('/M2/dji_sdk/odometry', Odometry, queue_size=10)
    

        main(velocity_publisher, velocity_message, odom_sim_pub_2, odometry_sim_msg_2)
